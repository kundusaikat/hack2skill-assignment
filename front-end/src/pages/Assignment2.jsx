...
import { getVideoData, searchVideoData } from "../functions/publicApi";
...
export const Assignment2 = () => {
  ...
  const fetchData = () => {
    if (searchQuery) {
      searchVideoData(searchQuery, currentPage, pageSize)
        .then((res) => {
      ...
      getVideoData(currentPage, pageSize)
        .then((res) => {
          const totalPages = Math.ceil(res.data.totalCount / pageSize);
          if (currentPage > totalPages) {
            setCurrentPage(totalPages);
  ...
  const columns = [
    ...
    columnHelper.accessor("thumbnails.default.url", {
      header: "Thumbnail",
      cell: (info) => (
        <img src={info.getValue()} alt="Thumbnail" width={120} height={90} />
      ),
      footer: (info) => info.column.id,
    }),
  ];
  ...
  useEffect(() => {
    fetchData();
  }, [currentPage, pageSize]); // add searchquery for onChange and change onclick to onchange

  return (
    <div>
      <p className="text-center">Assignment 2 - Youtube API</p>
